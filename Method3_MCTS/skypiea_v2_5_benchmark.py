...
class MyPlayer(PlayerDivercite):
    ...
    def compute_action(self, current_state: GameState, **kwargs) -> Action:
        ...
        # Initialisation du compteur d'états visités
        self.visited_states = 0
        
        # Appel de l'algorithme minimax avec le compteur d'états
        _, best_action = self.alpha_beta_minimax(current_state, depth, float('-inf'), float('inf'), True)
        
        # Affichage du nombre total d'états visités
        print(f"Nombre d'états visités : {self.visited_states}")
    ...
    def alpha_beta_minimax(self, state: GameState, depth: int, alpha: float, beta: float, maximizing_player: bool):
        # Incrémentation du compteur d'états visités
